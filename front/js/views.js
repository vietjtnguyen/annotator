var LineItemView = Backbone.View.extend({
  ...
  render: function() {
    var self = this;
    self.$el.html(self.template({index: 0}));
    return self;
  }
});

var LineListingView = Backbone.View.extend({
  collectionEvents: {
    "add": "addLine",
    "reset": "addAllLines",
    "all": "render"
  ...
  initialize: function() {
    var self = this;
    self.delegateCollectionEvents();
  },
  delegateCollectionEvents: function(collectionEvents) {
    var self = this;
    collectionEvents = collectionEvents === undefined ? self.collectionEvents : collectionEvents;
    _.forEach(collectionEvents, function(callback, trigger) {
      self.listenTo(self.collection, trigger, self[callback]);
    });
  },
  render: function() {
...
var AppView = Backbone.View.extend({
  ...
  create: function() {
    ...
    self.origin.append("svg:image")
      .attr("x", "0")
      .attr("y", "0")
      .attr("width", self.model.get("currentAnnotation").get("image").get("width"))
      .attr("height", self.model.get("currentAnnotation").get("image").get("height"))
      .attr("xlink:href", self.model.get("currentAnnotation").get("image").get("url"));
  ...
  resetView: function() {
    ...
    d3.transition().duration(400).tween("zoom", function() {
      ...
      ix = d3.interpolate(self.zoom.translate()[0], windowSize[0]/2 - self.model.get("currentAnnotation").get("image").width/2),
      iy = d3.interpolate(self.zoom.translate()[1], windowSize[1]/2 - self.model.get("currentAnnotation").get("image").height/2),
  ...
  canvasClick: function(domElement, datum, index) {
    ...
    if (!d3.event.ctrlKey) {
      ...
      app.get("currentAnnotation").get("lines").first().get("points").add(newPoint);
